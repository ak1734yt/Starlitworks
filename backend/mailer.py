...
def send_quote_email(name: str, to: str, order_id: int, service_name: str, quoted_price: float) -> bool:
    frontend_url = os.getenv("FRONTEND_URL", "http://localhost:5173")
    amount_str = f"Rs. {quoted_price:,.2f}" if not float(quoted_price).is_integer() else f"Rs. {quoted_price:,.0f}"
    html = f"""<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><style>
  body {{ background: #0a0a0a; color: #e5e7eb; font-family: 'Inter', sans-serif; margin: 0; padding: 0; }}
...
<body>
  <div class="container">
    <div class="header">
      <h1>💰 Price Request Ready</h1>
    </div>
    <div class="body">
      <p>Hey <strong style="color:#a78bfa">{name}</strong>! 👋</p>
      <p>We've reviewed your request for <strong>{service_name}</strong> (Order #{order_id}) and our team has prepared a price for you.</p>
      <p>Your quoted price is: <strong>{amount_str}</strong></p>
      <p>You can view the full details and proceed to payment or negotiate through your dashboard.</p>
      <a href="{frontend_url}/history" class="btn">View Quote & Pay →</a>
    </div>
    <div class="footer">
      <p>© 2026 Starlit Siege Works. All rights reserved.</p>
    </div>
    ...
    return _send_email(to, f"💰 Quote Ready for Order #{order_id}", html)
