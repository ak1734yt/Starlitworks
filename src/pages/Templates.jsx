  ...
  return (
    <div className="min-h-screen bg-brand-bg relative overflow-hidden flex flex-col justify-between">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-32 pb-24 w-full flex-1 relative z-10">
          ...
          <>
                ...
                <div className="bg-[#0A0A0A]/80 backdrop-blur-md p-1.5 rounded-full border border-white/10 flex items-center shadow-2xl">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const purchased = templates.filter(t => t.has_purchased);
                      setSelectedTemplate(purchased.length > 0 ? purchased[0] : null);
                    }}
                    className={`px-8 py-2.5 rounded-full text-xs font-bold transition-all ${
            ...
            {activeTab === "purchased" && templates.filter(t => t.has_purchased).length === 0 ? (
              <div className="text-center py-24 glass rounded-3xl border border-white/5 max-w-xl mx-auto">
                ...
                <button onClick={() => { setActiveTab("all"); if (templates.length > 0) setSelectedTemplate(templates[0]); }} className="btn-primary py-2.5 px-6 text-xs font-bold">
                  Browse Marketplace
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-4 space-y-4">
                  <h3 className="font-display font-bold text-xl mb-4 text-white">
                    {activeTab === "all" ? "Select Layout" : "My Purchased Layouts"}
                  </h3>
                  {(activeTab === "all" ? templates : templates.filter(t => t.has_purchased)).map((tpl) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2.5 mt-4 pt-3 border-t border-white/5">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                        ...
                        {!tpl.has_purchased ? (
                          ...
                          <button
                            ...
                            className="py-2 px-3 bg-green-500/10 border border-green-500/20 text-green-400 rounded-xl text-xs font-bold text-center cursor-default"
                          >
                            Purchased
                          </button>
                        )}
                ...
                {/* Right side: Interactive Discord Previewer */}
                {selectedTemplate && (
                  <div className="lg:col-span-8 space-y-6">
                    <div className="flex items-center justify-between">
                      <h3 className="font-display font-bold text-xl text-white">Virtual Discord Preview</h3>
                      {selectedTemplate.has_purchased ? (
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => {
                              navigator.clipboard.writeText(selectedTemplate.template_link);
                              toast.success("Discord template link copied!");
                            }}
                            className="px-4 py-2.5 bg-white/5 border border-white/10 hover:bg-white/10 rounded-xl text-white text-xs font-bold transition-all"
                          >
                            Copy Link
                          </button>
                          <a
                        ...
                        <button
                          ...
                          className="btn-primary py-2.5 px-6 text-sm flex items-center gap-2"
                        >
                          {purchaseLoading ? "Processing..." : "Request Blueprint Quote"}
                          <ArrowRight className="w-4 h-4" />
                        </button>
                      )}
                    </div>

                    {/* Virtual Discord Mock */}
                    <div className="w-full bg-[#1e1f22] rounded-3xl overflow-hidden border border-white/10 shadow-2xl flex aspect-video min-h-[450px]">
                      ...
                      <div className="w-60 bg-[#2b2d31] flex flex-col shrink-0 text-left border-r border-[#1e1f22]">
                        <div className="h-12 border-b border-[#1f2023] flex items-center justify-between px-4 font-bold text-white text-sm">
                      ...
                      {/* Panel 4: Roles panel */}
                      <div className="w-44 bg-[#2b2d31] shrink-0 text-left p-3 hidden md:flex flex-col border-l border-[#1e1f22]">
                        <p className="text-[10px] font-bold text-[#949ba4] uppercase tracking-wider mb-3 px-1">
        ...
        {/* Full-screen Interactive Discord Mock Previewer Modal */}
        {previewOpen && selectedTemplate && (
          <div className="fixed inset-0 bg-[#0A0A0C]/98 backdrop-blur-xl z-50 flex flex-col animate-fade-in text-[#dbdee1] overflow-hidden">
            {/* Top Header Bar */}
            ...
            <div className="flex-1 flex overflow-hidden flex-col lg:flex-row">
              {/* Sidebar Panel */}
              <div className="w-full lg:w-80 border-b lg:border-b-0 lg:border-r border-white/10 p-6 flex flex-col justify-between shrink-0 bg-[#1e1f22]">
                <div className="space-y-6 overflow-y-auto max-h-[30vh] lg:max-h-none">
                  <div>
