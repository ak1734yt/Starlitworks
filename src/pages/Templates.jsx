...
import { Sparkles, Hash, Volume2, Shield, ArrowRight, UserPlus, Info, Check, MessageSquare, X } from "lucide-react";
...
export default function Templates() {
  ...
  const handlePurchase = async (templateId) => {
    if (!user) {
      openAuthModal(`/checkout/template/${templateId}`, "login");
      return;
    }
    ...
    try {
      ...
      if (res.success && res.order_id) {
        navigate(`/checkout/${res.order_id}`);
      }
    } catch (err) {
      alert(err.message || "Failed to initiate template purchase.");
    } finally {
                    ...
                    >
                      <div className="flex justify-between items-start gap-4 mb-2">
                        ...
                        {tpl.has_purchased ? (
                          ...
                          <span className="font-mono font-black text-brand-secondary shrink-0 text-sm">₹{tpl.price}</span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {selectedTemplate.has_purchased ? (
                        ...
                        <button
                          ...
                          className="btn-primary py-2.5 px-6 text-sm flex items-center gap-2"
                        >
                          {purchaseLoading ? "Processing..." : `Order Blueprint • ₹${selectedTemplate.price}`}
                          <ArrowRight className="w-4 h-4" />
                        </button>
                      )}
                    </div>
